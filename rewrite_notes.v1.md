...
|---|---|---|---|---|---|---|---|---|---|
| Introduction | CLEAN | Cross-cutting 6 | 5 | 4 | 5 | 4 | 4 | 4 | 26 |
| Background | CLEAN | Cross-cutting 6 | 5 | 4 | 4 | 4 | 5 | 4 | 26 |
| Design | CLEAN | Cross-cutting 6 | 5 | 4 | 5 | 5 | 5 | 4 | 28 |
| Implementation | CLEAN | Pass 8 | 4 | 3 | 4 | 4 | 4 | 3 | 22 |
| Scenarios | CLEAN | Pass 1 (redistributed) | — | — | — | — | — | — | eliminated |
| Evaluation | CLEAN | Cross-cutting 6 | 5 | 5 | 4 | 4 | 5 | 3 | 26 |
| Discussion | CLEAN | Cross-cutting 6 | 5 | 4 | 3 | 4 | 5 | 3 | 24 |
| Conclusion | CLEAN | Cross-cutting 6 | 5 | 5 | 5 | 4 | 5 | 4 | 28 |
...
|---|---|---|---|---|---|---|---|
| Introduction | 5 | 4 | 4 | 4 | 4 | 3 | 24 |
| Background | 4 | 4 | 4 | 4 | 4 | 4 | 24 |
| Design | 4 | 4 | 4 | 5 | 5 | 3 | 25 |
| Implementation | 4 | 3 | 4 | 3 | 4 | 3 | 21 |
| Evaluation | 4 | 4 | 3 | 3 | 5 | 3 | 22 |
| Discussion | 4 | 4 | 4 | 4 | 5 | 3 | 24 |
| Conclusion | 4 | 5 | 5 | 4 | 5 | 3 | 26 |

**Priority ranking:** Implementation (21), Evaluation (22), Introduction (24), Background (24), Discussion (24), Design (25), Conclusion (26)

**Cross-cutting issues from grader:** 8 (2 HIGH concept re-introductions, 4 MEDIUM citation role duplications, 1 MEDIUM terminology inconsistency, 1 note on claim-evidence mapping — complete)

**Target:** Cross-cutting issues (all sections were CLEAN; re-grading surfaced value density problems across chapters)

**What I changed:**
- Ephemeral key lifecycle re-stated in Evaluation R2 section: replaced "rotated every five minutes" with cross-reference to Section 3.4.4 (criterion: C6)
- Ephemeral key lifecycle re-stated in Evaluation security posture section: replaced "generate, broadcast, rotate every five minutes, delete expired keys" enumeration with cross-reference to Section 3.4.4 (criterion: C6)
- Ephemeral key lifecycle re-explained in Discussion key leakage section: trimmed redundant "they exist only in mDNS TXT records, available only to devices currently on the network segment" — the preceding sentence already establishes keys never appear in files/env/repos (criterion: C6)
- VLC security surface duplicated between Implementation and Evaluation: replaced Evaluation's re-statement of "/shutdown endpoint" with cross-reference to Section 4.3.4 (criterion: C6)
- Redundant summary sentence in Implementation ("Each pattern trades integration depth for applicability breadth") removed — line 88 already establishes the ordering principle (criterion: C6)

**Not changed (grader noted but acceptable as-is):**
- Introduction's meli2019secrets/qazi2023api citations: already include explicit cross-reference to Background ("a pattern Chapter 2 documents across..."). The citations serve as forward pointers, not evidence duplication.
- Discussion's AP isolation section: does cross-reference Evaluation, and the "inverts the deployment map" framing plus Bassignana/Carmona-Galindo citations add NEW interpretive content not present in Evaluation. This is Discussion doing its job.
- Terminology "nine implementations" vs "seven consumers": the Introduction's Contributions section already clarifies the distinction. The grader acknowledged this is adequate.

**Remaining issues for future passes:**
- Implementation C2 (3/5): System Walkthrough section conflates implementation documentation with evaluative claims. The end-user walkthrough sentence ("AI capability is a feature of the application, not a service they configured") is a claim, not an implementation description.
- Implementation C4 (3/5): Open WebUI plugin and MCP tool decomposition lack design-intent framing.
- Evaluation C3 (3/5): Integration opportunity analysis ordering; R1/R2/R3 before establishing what "feasibility" means operationally.
- Evaluation C4 (3/5): Integration opportunity section repeats content from excluded Scenarios chapter.
- C6 across all sections (mostly 3/5): The "too cheap to meter" Strauss reference in Introduction is dropped after two sentences and never developed.

**Status:** Implementation and Evaluation marked NEEDS ANOTHER PASS. All others CLEAN.

## Implementation — Structural Pass 6
...
**What I changed:**
- System Walkthrough evaluative framing (Issue 1, HIGH): Removed "performing its core tasks" framing from section intro. Simplified Python figure caption to remove evaluative claim about runtime resolution. Replaced evaluative developer paragraph with artifact-level description of the two-function API. Removed "AI capability is a feature of the application, not a service they configured" from end user walkthrough. (C2)
- Open WebUI Plugin missing design intent (Issue 2, HIGH): Replaced evaluative opening ("tests whether a single file...") with constraint-first framing: Open WebUI's Pipe architecture constrains to single-file uploads. Added explicit trade-off: no compile-time dependency validation. (C4)
- Application Fork missing design intent (Issue 3, MEDIUM): Replaced "last resort" framing with design-intent statement: fork targets deepest integration surface (dynamic provider management, real-time model updates, multi-UI sync). Added explicit cost: version lock-in. (C4)
- Discovery Layer re-description (Issue 4, MEDIUM): Trimmed SaturnAdvertiser description to remove wrapping detail already in Design. Replaced "wraps zeroconf's ServiceInfo and register_service() calls" with direct statement. Shortened discover() description to cross-reference instead of re-narrate. (C6)
- Localhost Proxy evaluative claims (Issue 5, MEDIUM): Removed boundary-case generalization ("if the protocol works with an application whose only extensibility is a URL field, then the proxy pattern is a universal fallback"). Removed second paragraph re-stating proxy pattern value from taxonomy. Added cached-snapshot trade-off inline. (C6)
- Closing paragraph presupposes evaluation (Issue 6, MEDIUM): Replaced "evaluates whether these nine components...interoperate on the shared wire format" with neutral forward reference to claims from Section 1.4. (C2)

**Status:** CLEAN

## Evaluation — Structural Pass 6
...
**Issues from grader:** 7 cross-cutting issues affecting Evaluation (3 high, 4 medium)

**What I changed:**
- Resolution sequence restated at R1 (line 32): Removed "browse PTR, resolve SRV, parse TXT" restatement — Background and Design already established this flow. (C6)
- R2 opening restated protocol mechanics (lines 39-41): Cut redundant paragraph restating "connection sufficiency tests whether zero-config survives past discovery." Merged two paragraphs into one that goes straight to the table. Removed "README, .env file, Slack message" pattern already used in earlier chapters. (C6)
- Integration Opportunity advocacy tone (lines 166-168): Replaced "For 17 of the 42 targets, filling that gap requires importing one library and calling one function" with explicit qualification that analysis measures technical compatibility, not development effort. (C6)
- Redundant library independence restatement (line 129): Merged two paragraphs that both said "interoperability rests on libraries, not shared code" into one. (C6)
- Wharton double citation (lines 173/177): Removed citation from section intro (line 173), kept canonical citation at methodology definition (line 177). (C6)
- Nielsen double citation (lines 308/310): Removed second \cite{} from adjacent sentence — "Nielsen" alone suffices when just cited. (C6)
- Credential burden re-introduction (line 81): Replaced "The per-application credential burden documented in Section X disappears" with forward-looking "One schema handles three authentication models without per-application configuration." (C6)

**Status:** CLEAN

## Cross-cutting — Pass 6

**Grader scores (this cycle):**

| Section | C1 | C2 | C3 | C4 | C5 | C6 | Total |
|---|---|---|---|---|---|---|---|
| Introduction | 5 | 4 | 5 | 4 | 4 | 4 | 26 |
| Background | 5 | 4 | 4 | 4 | 5 | 4 | 26 |
| Design | 5 | 4 | 5 | 5 | 5 | 4 | 28 |
| Implementation | 4 | 3 | 4 | 4 | 4 | 3 | 22 |
| Evaluation | 5 | 5 | 4 | 4 | 5 | 3 | 26 |
| Discussion | 5 | 4 | 3 | 4 | 5 | 3 | 24 |
| Conclusion | 5 | 5 | 5 | 4 | 5 | 4 | 28 |

**Cross-cutting issues from grader:** 5 categories (concept re-introduction, citation role duplication, claim-evidence mapping, chapter inheritance, terminology consistency)

**Target:** Cross-cutting concept re-introductions (ephemeral key lifecycle, AP isolation). Also examined mDNS discovery re-introductions, "no URLs/keys/config" refrain, and Meli 81% — determined these already use correct cross-references or serve distinct rhetorical roles.

**What I changed:**
- Evaluation R2 section (line 60): Removed re-explanation of how clients use ephemeral keys as Bearer tokens. Kept cross-reference to Design Section 3.4.4; tightened to state the R2-relevant conclusion (no out-of-band provisioning) without re-deriving the mechanism. (C6: value density, principle 13: one canonical home)
- Discussion Section 6.3 (lines 51-52): Replaced re-derivation of ephemeral key benefits ("ensures a captured key expires before an attacker can build anything on it, and keys never appear in...") with tighter cross-reference ("Because no persistent credential exists") that lets the novel insight ("the inventory problem vanishes") land in the stress position. (C6: value density, principle 10: old-to-new)
- Discussion Section 6.5 third paragraph (lines 60-62): Removed restated material from Evaluation ("AP isolation is not solvable within Saturn's protocol", "Saturn works on its target networks") and replaced with interpretive content: the workarounds reintroduce the administrative overhead Saturn eliminates, and neither scales to multi-building campus networks. (C6: value density, discussion rule 1: interpret, do not restate)
- Design Section 5.2 (line 206): Removed forward reference to Evaluation results ("confirms the failure on two institutional deployments") which broke chapter ordering. Replaced with a brief forward pointer ("Section X tests this boundary") that doesn't reveal evaluation results in the Design chapter. (C3: ordering, chapter contract: Design hands off to Implementation, not Evaluation)

**What I did NOT change:**
- mDNS discovery re-introductions in Implementation (lines 40, 121, 141): Each cross-references Design and adds only implementation-specific detail (settle-time, polling intervals, async wrapping). Correct use of principle 13.
- "No URLs, no keys, no config" refrain: Each instance serves a different rhetorical role (Introduction: vision; Design Goals: requirement; Design Audiences: developer benefit). Not redundant.
- Meli 81% / static key fragility: Cross-references to Section~\ref{sec:key-fragility} are already correct throughout.
- Citation role duplication for rfc6762: Spec citations in different contexts (protocol definition, design justification, deployment constraint) serve different warrant roles.

**Implementation marked NEEDS ANOTHER PASS:** Grader scored it 22/30 (lowest). Issues include evaluative content in client integration subsections (C2), discovery re-explanation across subsections (C6), and the system walkthrough overlapping with Evaluation's cognitive walkthrough (C2). These are section-level structural issues requiring a dedicated Implementation pass, not cross-cutting fixes.

**Status:** Cross-cutting issues addressed. Implementation needs a structural pass next.

## Implementation — Structural Pass 7
...
**What I changed:**
- Issue 1 (HIGH/C2): Walkthroughs made evaluative assertions. Removed \argref tags from walkthrough subsections and reframed "No further configuration is needed—every device receives the beacon automatically" as operational description: "the mDNS beacon broadcasts on the local network; any device running a Saturn client receives the service advertisement without additional setup."
- Issue 2 (HIGH/C6): Discovery Layer re-narrated protocol flow from Design Section 3.5.4. Trimmed re-explanation; kept only implementation-specific decisions (settle-time, async browser, collision avoidance) with bare cross-reference.
- Issue 3 (MEDIUM/C6): Ollama server paragraph re-enumerated endpoint paths from Design Table 3.2. Replaced endpoint path listing with cross-reference to TXT schema table; led with the translation decision (Ollama native → OpenAI-compatible).
- Issue 4 (MEDIUM/C4): Jan subsection lacked rationale for proxy pattern. Added explicit trade-off statement: proxy trades staleness and a background process for compatibility with any application accepting a base URL, requiring zero target modification.
- Issue 5 (MEDIUM/C2): Pattern taxonomy contained evaluative comparison ("Deeper patterns yield better runtime behavior...but impose stricter requirements"). Removed evaluative sentence; taxonomy now presents patterns descriptively without comparing quality.
- Issue 6 (MEDIUM/C5): Native SDK subsection omitted no-service failure mode and dependency cost. Added explicit failure description: no Saturn services on network renders the provider inert with no fallback to manual configuration.
- Issue 7 (MEDIUM/C3): OpenCode fork not in six-pattern taxonomy, rationale delayed. Added opening explanation that the fork falls outside the taxonomy as a seventh pattern, and led with the constraint (no plugin system, no tool protocol) that forced source modification.
- Issue 8 (MEDIUM/C6): Aider subsection described Aider before Saturn wrapper. Reordered to lead with the Saturn wrapper's purpose (inject discovered credentials into unmodified tool), then state Aider's mechanism in subordinate position. Added user scenarios for each optional flag.
- Factual error: Binary size stated as "approximately 700 KB" but moons/graph.json and moons/claims/feasibility/evidence.md both record ~2MB. Corrected to "approximately 2 MB."

**Status:** NEEDS ANOTHER PASS

## Implementation — Structural Pass 8
...
**What I changed:**
- Issue 1 (HIGH/C2): Removed "Saturn's original contribution is..." evaluative language from line 29. Replaced with factual separation of reused infrastructure vs. new code, plus a roadmap sentence. (criterion: C2)
- Issue 2 (HIGH/C2): Replaced evaluative closing "evaluates these nine components against the three claims" with neutral transition "the system is ready for the evaluation." (criterion: C2)
- Issue 3 (HIGH/C6): Varied the six-item integration pattern enumeration. Each item now uses a different rhetorical structure: some lead with cost, some with constraint, some with scope. Eliminated the uniform "benefit---but cost" template. (criterion: C6)
- Issue 4 (MEDIUM/C6): Replaced re-explanation of latency/completeness trade-off in settle-time paragraph with cross-reference to Section 3.5.4 (Design). Kept the implementation-specific one-second value. (criterion: C6)
- Issue 5 (MEDIUM/C4): Restructured Jan/localhost proxy subsection to lead with design intent (targets applications whose only extensibility surface is a base URL) before introducing Jan as the specific case. (criterion: C4)
- Issue 6 (MEDIUM/C3): Reordered OpenCode subsection to lead with the constraint (no plugin system, no tool protocol) before listing OpenCode's architecture. Reader learns why the subsection exists before encountering the application's features. (criterion: C3)
- Issue 7 (MEDIUM/C6): Trimmed walkthrough prose that duplicated earlier subsections. Removed redundant framing sentences, VLC metadata extraction already described in Section 4.3.4. Kept terminal examples. (criterion: C6)
- Issue 8 (MEDIUM/C5): Added shadowing trade-off to configuration subsection: user overrides persist across package upgrades, so schema changes in built-in configurations do not propagate to customized files. (criterion: C5)
