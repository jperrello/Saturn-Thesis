...
\textbf{Desc:} A short one sentence description of what the element is. Example: the ephemeral-keys description reads ``Time-limited API credentials (10-min lifetime, 5-min rotation with overlap for zero-downtime transitions) distributed via mDNS TXT records and deleted on shutdown, paralleling Kerberos session-layer distribution.''
...
Figure~\ref{fig:moon-chunk} shows an example moon chunk.
...
\small
\smallskip
\texttt{\{"id": "mdns", "type": "concept",} \\
\texttt{\ "desc": "Link-local name resolution (RFC 6762)} \\
\texttt{\ \ via multicast to 224.0.0.251:5353 requiring no} \\
\texttt{\ \ DNS server -{}- Saturn's protocol foundation.",} \\
\texttt{\ "file": "concepts/mdns.md",} \\
\texttt{\ "edges": [} \\
\texttt{\ \ \ \ \{"to": "claim-1", "rel": "supports"\},} \\
\texttt{\ \ \ \ \{"to": "dns-sd",\ \ "rel": "related"\},} \\
\texttt{\ \ \ \ \{"to": "ch2",\ \ \ \ \ "rel": "discussed\_in"\},} \\
\texttt{\ \ \ \ \{"to": "ch3",\ \ \ \ \ "rel": "discussed\_in"\}} \\
\texttt{]\}}
\smallskip
\end{singlespace}
\end{minipage}}
\caption{A single moon chunk from \texttt{graph.json}. The \texttt{desc} field provides a one-sentence summary; the \texttt{edges} array encodes typed relationships to other nodes.}
\label{fig:moon-chunk}
\end{figure}
...
Figure~\ref{fig:moons-claude} shows the moons instructions from CLAUDE.md.
...
\small
\smallskip
\texttt{\#\#\# Moons (Knowledge Base)}

This project uses a structured knowledge graph called \texttt{moons/} to organize all thesis context.

\medskip
\textbf{Structure:}
\begin{itemize}
    \item \texttt{moons/graph.json} --- the map. Lightweight nodes with typed edges. Read this FIRST to navigate.
    \item \texttt{moons/concepts/} --- Saturn concepts (mDNS, DHCP, ephemeral keys, etc.)
    \item \texttt{moons/claims/} --- the 3 thesis claims with evidence
    \item \texttt{moons/papers/} --- distilled paper summaries with relevance to Saturn
    \item \texttt{moons/voice/} --- user's personal perspective, design rationale, interview notes
    \item \texttt{moons/code/} --- code component analyses with line citations
    \item \texttt{moons/chapters/} --- chapter planning and section status
    \item \texttt{moons/meta/} --- advisor notes, timeline, project context
\end{itemize}

\medskip
\textbf{How to use:}
\begin{enumerate}
    \item Always read \texttt{graph.json} first. At \textasciitilde500 lines it gives complete graph visibility.
    \item Follow edges to specific content files only when you need depth.
    \item After new information, update both the relevant content file AND \texttt{graph.json} edges.
    \item Keep \texttt{graph.json} lean --- short descriptions, not full content.
    \item \texttt{moons/query.py} exists as a human CLI convenience.
\end{enumerate}

\medskip
\textbf{Moons is the workspace. \texttt{thesis.tex} is the submission.}
\smallskip
\end{singlespace}
\end{minipage}}
\caption{The moons instructions from CLAUDE.md, providing agents with the knowledge graph's structure and usage conventions.}
\label{fig:moons-claude}
\end{figure}
...
\smallskip
\texttt{while :; do} \\
\texttt{\ \ \ \ cat RALPH\_PROMPT.md | claude -p --dangerously-skip-permissions} \\
\texttt{\ \ \ \ sleep 2} \\
\texttt{done}
\smallskip
\end{singlespace}
\end{minipage}}
\captionof{figure}{The refined Ralph loop. Each iteration pipes the orchestrator prompt into a fresh Claude session with full permissions, then pauses briefly before the next pass.}
\label{fig:ralph-loop}
\end{center}
